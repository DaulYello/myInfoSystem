layui.use(['layer', 'table', 'ax', 'laydate'], function () {
    ...
    LoginLog.logDetail = function (param) {
        var ajax = new $ax(Huang.ctxPath + "/log/detail/" + param.operationLogId, function (data) {
            Huang.infoDetail("日志详情", data.regularMessage);
        }, function (data) {
            Huang.error("获取详情失败!");
        });
        ajax.start();
    ...
    LoginLog.cleanLog = function () {
        Huang.confirm("是否清空所有日志?", function () {
            var ajax = new $ax(Huang.ctxPath + "/log/delLog", function (data) {
                Huang.success("清空日志成功!");
                LoginLog.search();
            }, function (data) {
                Huang.error("清空日志失败!");
            });
